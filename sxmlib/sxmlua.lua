...
local requestsDisabled = true --getgenv and getgenv().DISABLE_SXMCODE_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local SXMCODEFolder = "SXMCODE"
local ConfigurationFolder = SXMCODEFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		SXMCODEOpen = {Type = 'bind', Value = 'K', Name = 'SXMCODE Keybind'},
		-- buildwarnings
		-- SXMCODEprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.SXMCODEOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(SXMCODEFolder) then
				if isfile and isfile(SXMCODEFolder..'/settings'..ConfigurationExtension) then
					file = readfile(SXMCODEFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"SXMCODEOpen":{"Value":"K","Type":"bind","Name":"SXMCODE Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"SXMCODE Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('SXMCODE had an issue accessing configuration saving capability.')
		end
	end
end
...
local SXMCODE = useStudio and script.Parent:FindFirstChild('SXMCODE') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local SXMCODEDestroyed = false -- True when SXMCODELibrary:Destroy() is called

repeat
	if SXMCODE:FindFirstChild('Build') and SXMCODE.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('SXMCODE | Build Mismatch')
		print('SXMCODE may encounter issues as you are running an incompatible interface version ('.. ((SXMCODE:FindFirstChild('Build') and SXMCODE.Build.Value) or 'No Build') ..').\n\nThis version of SXMCODE is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, SXMCODE = SXMCODE, useStudio and script.Parent:FindFirstChild('SXMCODE') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

SXMCODE.Enabled = false

if gethui then
	SXMCODE.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(SXMCODE)
	SXMCODE.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	SXMCODE.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SXMCODE.Name and Interface ~= SXMCODE then
			Interface.Enabled = false
			Interface.Name = "SXMCODE-Old"
		end
	end
elseif not useStudio then
...
if SXMCODE.AbsoluteSize.X < minSize.X and SXMCODE.AbsoluteSize.Y < minSize.Y then
...
local Main = SXMCODE.Main
local MPrompt = SXMCODE:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = SXMCODE:FindFirstChild('Drag')
...
local Notifications = SXMCODE.Notifications

local SelectedTheme = SXMCODELibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = SXMCODELibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	SXMCODE.Main.BackgroundColor3 = SelectedTheme.Background
	SXMCODE.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	SXMCODE.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	SXMCODE.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	SXMCODE.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	SXMCODE.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	SXMCODE.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		SXMCODE.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		SXMCODE.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(SXMCODE:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("SXMCODE | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("SXMCODE | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('SXMCODE had an issue decoding the configuration file, please try delete the file and reopen SXMCODE.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(SXMCODELibrary.Flags) do
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(SXMCODELibrary.Flags) do
...
function SXMCODELibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			SXMCODELibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			SXMCODELibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "SXMCODEOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(SXMCODEFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('SXMCODE Settings', 0, true)

	if TabList['SXMCODE Settings'] then
		TabList['SXMCODE Settings'].LayoutOrder = 1000
	end

	if Elements['SXMCODE Settings'] then
		Elements['SXMCODE Settings'].LayoutOrder = 1000
	end
...
function SXMCODELibrary:CreateWindow(Settings)
	print('creating window')
	if SXMCODE:FindFirstChild('Loading') then
		if getgenv and not getgenv().SXMCODECached then
			SXMCODE.Enabled = true
			SXMCODE.Loading.Visible = true

			task.wait(1.4)
			SXMCODE.Loading.Visible = false
		end
	end

	if getgenv then getgenv().SXMCODECached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				SXMCODELibrary:Notify({Title = 'Build Mismatch', Content = 'SXMCODE may encounter issues as you are running an incompatible interface version ('.. ((SXMCODE:FindFirstChild('Build') and SXMCODE.Build.Value) or 'No Build') ..').\n\nThis version of SXMCODE is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "SXMCODE"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "SXMCODE Interface Suite" then
		LoadingFrame.Version.Text = "SXMCODE UI"
	end
	...
	if not Settings.DisableSXMCODEPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(SXMCODEFolder.."/Discord Invites") then
			makefolder(SXMCODEFolder.."/Discord Invites")
		end

		if isfile and not isfile(SXMCODEFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(SXMCODEFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"SXMCODE RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(SXMCODEFolder.."/Key System") then
			makefolder(SXMCODEFolder.."/Key System")
		end
		...
		if isfile and isfile(SXMCODEFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(SXMCODEFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SXMCODEFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						SXMCODELibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called SXMCODELibrary:Destroy()
				if SXMCODEDestroyed then
					return
				end
				if not Success then
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= SXMCODELibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SXMCODELibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			SXMCODE.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					SXMCODELibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SXMCODELibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SXMCODELibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= SXMCODELibrary.Theme.Default then
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						SXMCODELibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SXMCODELibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			SXMCODELibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			SXMCODELibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('SXMCODE had an issue creating settings.') end
...
function SXMCODELibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function SXMCODELibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function SXMCODELibrary:Destroy()
	SXMCODEDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['SXMCODE Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "SXMCODEOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function SXMCODELibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				SXMCODELibrary:Notify({Title = "SXMCODE Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			SXMCODELibrary:Notify({Title = "SXMCODE Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('SXMCODE Configurations Error | '..tostring(result))
			SXMCODELibrary:Notify({Title = "SXMCODE Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = SXMCODELibrary:CreateWindow({
	--	Name = "SXMCODE Example Window",
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using SXMCODE may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like SXMCODE to get the key from
	...
	----SXMCODELibrary:Notify({Title = "SXMCODE Interface", Content = "Welcome to SXMCODE. These - are the brand new notification design for SXMCODE, with custom sizing and SXMCODE calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(SXMCODELibrary.Theme) do
...
task.delay(4, function()
	SXMCODELibrary.LoadConfiguration()
